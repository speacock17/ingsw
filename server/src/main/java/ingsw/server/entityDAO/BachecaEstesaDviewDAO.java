...
public class BachecaEstesaDviewDAO {
    ...
    public BachecaEstesaDviewDAO(Connection conn) throws SQLException {
        connection = conn;

        // compilo le query
        recuperaTuttaBachecaPS = connection.prepareStatement("SELECT * FROM BACHECAESTESAD");
        recuperaBachecaPS = connection.prepareStatement("SELECT * FROM BACHECAESTESAD WHERE Destinatario = ?");
        recuperaVisibiliPS = connection.prepareStatement("SELECT * FROM BACHECAESTESAD WHERE Nascosto = false AND Destinatario = ?");
        recuperaNascostiPS = connection.prepareStatement("SELECT * FROM BACHECAESTESAD WHERE Nascosto = true AND Destinatario = ?");
        recuperaVistiPS = connection.prepareStatement("SELECT * FROM BACHECAESTESAD WHERE Visualizzato = true AND Nascosto = false AND Destinatario = ?");
        recuperaVistiNascostiPS = connection.prepareStatement("SELECT * FROM BACHECAESTESAD WHERE Visualizzato = true AND Nascosto = true AND Destinatario = ?");
    }
